...
- **Shell 调试**: `link can/lora` (tab 补全切换 CAN/LoRa) + `lora send/at/exit` 基础命令 (at 支持直接输入 `AT+` 前缀) + `lora gw config [prot] [mode] [spd] [ch]` 通信参数配置 + `lora gw query` 通信参数查询 + `lora nid [hex]` 查询/设置节点 ID + `lora gwid [hex]` 查询/设置网关 ID。

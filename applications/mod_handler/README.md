...
- **CAN/LoRa 手动切换** — 通过 link_switch 按键 (PA10) 手动切换 CAN/LoRa 连接类型，启动默认 CAN
...
| LoRa 心跳 | 1024 | 12 | 仅 LoRa 模式运行，2s 周期遥测 + ACK 检测 |
...
- **信号量**: `k_sem` 配合心跳超时检测 + LoRa ACK 检测
...
```
Row 0: [信号图标][电池图标][CAN/LORA图标] NID        ← 图标 + 节点 ID
...
- **电池图标** (24x16): 4 级电量图标 (25%/50%/75%/100%)，充电时显示闪电标记
...
1. 按下 link_switch → CAN ↔ LoRa 切换
2. 切换时即时刷新 OLED Row 0
3. CAN 模式：角度/按键变化时通过 CAN 发送，心跳线程周期发送手柄状态帧
4. LoRa 模式：角度/按键变化时通过 LoRa 发送遥测帧，LoRa 心跳线程维护链路检测
...
```
include/
  common.h          -- 全局状态类型 (gloval_params_t, scanner_data_t, CAN_TYPE/LORA_TYPE, nid/gwid/rssi)
  mod-can.h         -- CAN 协议定义 + OTA 接口
  lora.h            -- LoRa 驱动接口 (透传/AT/遥测/网关配置/链路检测)

...
static int get_file(char *path, bool latest, int *file_num)
{
	...
	while ((ptr = readdir(dir)) != NULL) {
		if ((strncmp(ptr->d_name, "data_", strlen("data_")) != 0)) {
			continue;
		}
		if (snprintf(path, 128, ROOT "/%s", ptr->d_name) < 0) {
			return -1;
		}
		if (sscanf(ptr->d_name, "data_%u.raw", &tmp) != 1) {
			continue;
		}
		...
		if (stat(path, &st) == 0) {
			...
			if (val == 0) {
				val = tmp;
				size = st.st_size;
				if (snprintf(file_name, sizeof(file_name), "%s", ptr->d_name) < 0) {
					return -1;
				}
			} else if (latest) {
				if (val < tmp) {
					val = tmp;
					size = st.st_size;
					if (snprintf(file_name, sizeof(file_name), "%s",
						     ptr->d_name) < 0) {
						return -1;
					}
				}
	...
	if (snprintf(path, 128, ROOT "/%s", file_name) < 0) {
		return -1;
	}
	if (file_num) {
		*file_num = num;
...
static int open_latest_file(bool create)
{
	...
	/* create new file */
	{
		...
		if (snprintf(file_name, sizeof(file_name), ROOT "/data_%u.raw", t) < 0) {
			return -1;
		}

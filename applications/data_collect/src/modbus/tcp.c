		...
		if (FD_ISSET(serv, &readfds)) {
			...
			for (int i = 0; i < MAX_CLIENTS; i++) {
				if ((mb_clients[i].time && (mb_clients[i].time + 30000) < k_uptime_get()) ||
					...
					LOG_INF("Host(%s:%d) connection is terminated due to "
						"timeout by others",
			...
			for (int i = 0; i < MAX_CLIENTS; i++) {
				if (FD_ISSET(mb_clients[i].fd, &readfds)) {
					if ((rc = recv(mb_clients[i].fd, data_buf,
						       MODBUS_MBAP_AND_FC_LENGTH, MSG_WAITALL)) ==
					    0) {
						getpeername(mb_clients[i].fd,
							    (struct sockaddr *)&client_addr,
							    (socklen_t *)&client_addr_len);
						...
						mb_clients[i].time = k_uptime_get();
						LOG_HEXDUMP_DBG(data_buf, MODBUS_MBAP_AND_FC_LENGTH,
								"h:>");
						modbus_raw_get_header(&tmp_adu, data_buf);
						heart_event_send();
						data_len = tmp_adu.length;
						if ((rc = recv(mb_clients[i].fd, tmp_adu.data,
							       tmp_adu.length, MSG_WAITALL) < 0)) {
							LOG_ERR("receive data error");
							break;
						}

						LOG_HEXDUMP_DBG(tmp_adu.data, tmp_adu.length,
								"d:>");
						...
						memcpy(data_buf + MODBUS_MBAP_AND_FC_LENGTH,
						       tmp_adu.data, tmp_adu.length);

						if (send(mb_clients[i].fd, data_buf,
							 MODBUS_MBAP_AND_FC_LENGTH + tmp_adu.length,
							 0) < 0) {
							LOG_ERR("send error");
						}
					}
...
K_THREAD_DEFINE(mb_tcp, CONFIG_MODBUS_TCP_STACK_SIZE, tcp_poll, NULL, NULL, NULL,
		CONFIG_MODBUS_TCP_PRIORITY, 0, 0);

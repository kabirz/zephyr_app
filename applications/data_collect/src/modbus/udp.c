...
static int parse_udp_msg(uint8_t *msg, size_t len)
{
    ...
    if (ret > 0 && g_info.get_device_info == true) return get_d_info(msg, len);
	...
	if (s_info.set_device_info.slave_id) {
    	    update_holding_reg(HOLDING_SLAVE_ID_IDX, (uint16_t)s_info.set_device_info.slave_id);
    	    reg_changed = true;
	} 
	...
	if (s_info.set_device_info.rs485_bps) {
    	    update_holding_reg(HOLDING_RS485_BPS_IDX, (uint16_t)s_info.set_device_info.rs485_bps);
    	    reg_changed = true;
	}
	if (reg_changed) {
#ifdef CONFIG_SETTINGS
...
static void udp_poll(void)
{
    ...
    while (1) {
    	...
    	buf_len = recvfrom(serv, udp_buffer, sizeof(udp_buffer), 0, (struct sockaddr *)&client_addr, &client_addr_len);
    	if (buf_len > 0) {
        ...
        if (sendto(serv, udp_buffer, buf_len, 0, (struct sockaddr *)&client_addr, sizeof(struct sockaddr)) == -1) {

			...
			if (get_holding_reg(HOLDING_AI_EN_IDX) & (1 << i)) {
				...
				if (err < 0) {
					...
					if (i < 2) {
						...
						LOG_DBG("val[%d]: %d.%d mA", i, val_mv / 100,
							val_mv % 100);
					} else {
						val_mv = (int)(3.7037 * val_mv / 10);
						LOG_DBG("val[%d]: %d.%d mV", i, val_mv / 100,
							val_mv % 100);
					}
#else
...
K_THREAD_DEFINE(adc_io, CONFIG_MODBUS_ADC_STACK_SIZE, adc_handler, NULL, NULL, NULL,
		CONFIG_MODBUS_ADC_PRIORITY, 0, 0);
